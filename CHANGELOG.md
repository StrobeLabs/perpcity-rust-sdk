...
- **`PerpClient::poll_receipt(tx_hash)` is public** — the send path's own wait (2 s delay, then a 2 s poll for up to 30 s), returning `ReceiptTimeout` for the same hash on timeout so the call can repeat. It does not touch nonce tracking: a send that returned a hash has already stopped tracking it, and a doubtful nonce resyncs from chain before the next send either way.
