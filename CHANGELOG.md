...
- **`history::token_transfers(provider, token, from, to, from_block, to_block)`** — every ERC-20 `Transfer` of `token` whose sender is in `from` and whose recipient is in `to`, in chain order, read with `get_logs_chunked`. The sets go to the node as topic filters; an empty set matches any address, and two empty sets (or the zero token) are `ValidationError::InvalidConfig`, since the query would read every transfer the token made. Each result is a `TokenTransfer { block_number, log_index, tx_hash, from, to, value }`; `value` is raw token units. A log that does not decode as an ERC-20 `Transfer` (an ERC-721 `Transfer` shares the topic) is `ValidationError::DecodeFailed`, not a silent gap. Golden-tested against a real Arbitrum One USDC transfer. A treasury reads every transfer between itself and its wallets with two calls, and rebuilds its books from chain after a restart.
