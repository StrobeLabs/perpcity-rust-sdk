...
- **`prelude` module.** `use perpcity_sdk::prelude::*;` bundles the everyday public surface (client/transport, errors, gas/urgency, feeds, client-facing params/results, maker-equity types, liquidity sizing) behind one import. Every item is also still available individually at the crate root; lower-level ABI/contract-interface types and the fine-grained math helpers (`math::tick`, `math::swap`, `convert`) are deliberately not included.
