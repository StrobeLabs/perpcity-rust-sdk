...
- **`PerpFactoryRedeployEvents::PerpCreated`** — the `PerpCreated` shape of the redeployed factory (perpcity-contracts main `IPerpFactory`): adds `uint32 latency`, drops the position-NFT `name`/`symbol`/`tokenUri` strings. No factory emits it yet; it is declared so discovery reads a new factory from its first block while the old factory's perps stay live. Both factory shapes and `IBeacon::IndexUpdated` have their topic0 locked in `abi_lock` against the live chain.
- **`discovery::decode_perp_created(&Log) -> Option<PerpCreation>`** — decodes a `PerpCreated` log of either factory shape into one `#[non_exhaustive]` `PerpCreation` (perp, factory, block, pool id, modules, initial index, EMA window, protocol fee, starting sqrt price and tick, owner). `latency` is `Some` only for the redeployed factory; `metadata` (`PerpMetadata { name, symbol, token_uri }`) only for the deployed one. A pending log (no block number) is not a creation. Golden-tested against the real HORMUZ `PerpCreated` log (committed as `tests/fixtures/perp_created_arbitrum_one.json`). `contracts::Modules` now derives `Debug`, `PartialEq` and `Eq`.
- **`discovery::list_perps(provider, factories, from_block)`** — every perp the listed factories created from `from_block` to `SNAPSHOT_BLOCK_LAG` blocks behind the head (the newest blocks belong to the feed), oldest first, read with `get_logs_chunked` and filtered to both `PerpCreated` topic0s. The factory list comes from the caller (a redeploy adds a factory while the old one's perps stay live); an empty list or a zero address is `ValidationError::InvalidConfig`, since an unfiltered query would accept `PerpCreated` from any contract.
- **`feeds::PerpCreatedFeed`** (re-exported at the crate root) — a WebSocket subscription to the `PerpCreated` logs of the same factory list, both shapes, yielding each new market as a `PerpCreation`; logs removed by a reorg are skipped. Subscribe first, then `list_perps`, and merge by perp address to follow the list without a gap.
- **`history::get_logs_chunked(provider, filter, from_block, to_block)`** — every log matching a filter across a block range of any length, in chain order. Providers cap `eth_getLogs` by span, result count or response size and word the rejection differently, so the scan does not parse range messages: it halves a range the server rejects, doubles the span after each accepted range, and narrows in on the limit between the widest accepted and narrowest rejected span. A rejection of a span accepted before (a result cap in a dense stretch) drops what was learned, and a rejected span is retested after a run of accepted requests, so the scan widens again past a dense stretch. Failures no narrower range fixes are returned at once: no answer, a rate limit (HTTP 429/503, or a JSON-RPC error alloy's retry rules call one; `-32005` only when its message says so), a method or parse error (`-32601`, `-32700`), and HTTP 401/403. A single block that is still rejected is returned as the error.
- **Beacon print series.** `history::beacon_prints(provider, beacon, from_block, to_block)` returns every `IndexUpdated` print in a range, and `history::latest_beacon_prints(provider, beacon, from_block, to_block, limit)` the newest `limit`, reading backward from `to_block` and stopping once it has them (so `from_block` is only a floor). Both return `IndexPrint { block_number, log_index, timestamp, index_x96 }` oldest first, with `index()` for the float value. Timestamps come from the log's `blockTimestamp` when the provider sends it; otherwise each distinct block's header is read once, with bounded concurrency. The deployed beacons have no last-update getter (`index()` returns the value alone), so `latest_beacon_prints(.., 1)` is how to read when a beacon last printed.
